...
public class TransactionServiceTest
{
  ...
  public async Task Should_Publish_And_Return_Success_When_Partner_Is_Verified()
  {
    ...
    var result = await service.CreateTransaction(validRequest, CancellationToken.None);

    Assert.True(result.Success);
    await _queuePublisher.Received(1).PublishTransactionAsync(
        Arg.Is<TransactionMessage>(m => m.PartnerId == validRequest.PartnerId
  ...
  public async Task Should_Throw_ValidationException_When_Request_Is_Invalid()
  {
    ...
    await Assert.ThrowsAsync<ValidationException>(
        () => service.CreateTransaction(ValidRequest(), CancellationToken.None));
    ...
    await _queuePublisher.DidNotReceive().PublishTransactionAsync(Arg.Any<TransactionMessage>(), Arg.Any<CancellationToken>());
  }
